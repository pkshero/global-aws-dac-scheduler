...
## Pre-Requisite(S):						##
## 		- Git, AWS CLI, JQ 					##
##		- AWS Profile Setup as global		##
##############################################
DATE_WITH_TIME=$(date "+%Y%m%d-%H%M%S")
START=$(date +%s)
LOG_FILE=GDAC_SCHEDULER_$DATE_WITH_TIME.log
(
...
Create_GDAC_ES_SecurityGroup() {
    ...
    CHECK_SG=$(aws ec2 describe-security-groups --filters Name=group-name,Values=*global-labs-SG-$REGION* Name=vpc-id,Values=$VPC_ID --query "SecurityGroups[*].{Name:GroupName,ID:GroupId}" --profile $PROFILE --region $REGION | jq -r '.[].ID')
    if [[ $CHECK_SG == "" ]] || [[ $CHECK_SG == "null" ]]; then
        echo "INFO ::: Security Group Does Not Exist !!!"
        CREATE_SG=$(aws ec2 create-security-group --group-name "global-labs-SG-$REGION" --description "GDAC-ES Infrastructure security group" --profile $PROFILE --region $REGION --vpc-id $VPC_ID --tag-specifications 'ResourceType=security-group,Tags=[{Key=Application,Value=SecurityGroup for Global Analytics Connector Integration with AWS OpenSearch},{Key=PublicationType,Value=Global Labs},{Key=Developer,Value=Global},{Key=Version,Value=V 0.1}]')
	...
	GDAC_ES_SECURITYGROUP_ID="$SG_ID"
...
check_if_opensearch_exists(){
	...
	echo "INFO ::: GDAC_ES_SecurityGroup :: $GDAC_ES_SECURITYGROUP_ID"
	...
	if [ "$IS_ES" == "N" ]; then
		echo "INFO ::: Amazon_OpenSearch_Service is Not Configured. Need to Provision Amazon_OpenSearch_Service Before, GDAC Provisioning."
		...
		### GITHUB_ORGANIZATION defaults to global-labs
		if [ "$USE_PRIVATE_IP" == "N" ] || [ "$USE_PRIVATE_IP" == null ] || [ "$USE_PRIVATE_IP" == "" ]; then
			REPO_FOLDER="global-awsopensearch-public"
		else
			REPO_FOLDER="global-awsopensearch"
		fi
		validate_github $GITHUB_ORGANIZATION $REPO_FOLDER
		########################### Git Clone  ###############################################################
		echo "INFO ::: BEGIN - Git Clone !!!"
		### Download Provisioning Code from GitHub
		GIT_REPO_NAME=$(echo ${GIT_REPO} | sed 's/.*\/\([^ ]*\/[^.]*\).*/global-\1/' | cut -d "/" -f 2)
		...
		echo "gdac_es_securitygroup_id="\"$GDAC_ES_SECURITYGROUP_ID\" >>$OS_TFVARS
		echo "" >>$OS_TFVARS
		...
		echo "INFO ::: BEGIN ::: GDAC Provisioning . . . . . . . . . . . ."
	fi
...
check_if_kendra_exists(){
	...
	echo "INFO ::: GDAC_ES_SECURITYGROUP_ID : $GDAC_ES_SECURITYGROUP_ID"
	...
	if [ "$IS_KENDRA" == "N" ]; then
		echo "INFO ::: Amazon_Kendra_Service is Not Configured. Need to Provision Amazon_Kendra_Service Before, GDAC Provisioning."
		echo "INFO ::: Begin Amazon_Kendra_Service Provisioning."
		REPO_FOLDER="global-amazonkendra"
		validate_github $GITHUB_ORGANIZATION $REPO_FOLDER
		echo "INFO ::: BEGIN - Git Clone !!!"
		### Download Provisioning Code from GitHub
		GIT_REPO_NAME=$(echo ${GIT_REPO} | sed 's/.*\/\([^ ]*\/[^.]*\).*/global-\1/' | cut -d "/" -f 2)
...
validate_github() {
	...
	if [ "$GITHUB_ORGANIZATION" != "" ]; then
		...
		GITHUB_ORGANIZATION="global-labs"
...
gmc_endpoint_accessibility() {
	GDAC_SCHEDULER_NAME="$1"
	GDAC_SCHEDULER_IP_ADDR="$2"
    GMC_API_ENDPOINT="$3"
	GMC_API_USERNAME="$4"
	GMC_API_PASSWORD="$5" #14-19
	PEM="$PEM_KEY_PATH"
	
	chmod 400 $PEM
	### gdac_scheduler_name = from FourthArgument of GDAC_Scheduler.sh, user_sec.txt
	### parse_textfile_for_user_secret_keys_values user_sec.txt
	echo "INFO ::: GDAC_SCHEDULER_NAME ::: ${GDAC_SCHEDULER_NAME}"
	echo "INFO ::: GDAC_SCHEDULER_IP_ADDR ::: ${GDAC_SCHEDULER_IP_ADDR}"
	echo "INFO ::: GMC_API_ENDPOINT ::: ${GMC_API_ENDPOINT}"
	
	echo "INFO ::: GDAC_SCHEDULER_IP_ADDR : "$GDAC_SCHEDULER_IP_ADDR
	py_file_name=$(ls check_gmc_visiblity.py)
	echo "INFO ::: Executing Python code file : "$py_file_name
	cat $py_file_name | ssh -i "$PEM" ubuntu@$GDAC_SCHEDULER_IP_ADDR -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null python3 - $GMC_API_USERNAME $GMC_API_PASSWORD $GMC_API_ENDPOINT
	if [ $? -eq 0 ]; then
		echo "INFO ::: GDAC Scheduler with IP : ${GDAC_SCHEDULER_IP_ADDR}, have access to GMC API ${GMC_API_ENDPOINT} "
	else
		echo "ERROR ::: GDAC Scheduler with IP : ${GDAC_SCHEDULER_IP_ADDR}, Does NOT have access to GMC API ${GMC_API_ENDPOINT}. Please configure access to GMC "
		exit 1
	fi
	echo "INFO ::: Completed GMC endpoint accessibility Check. !!!"

}
parse_4thArgument_for_gdac_scheduler_name() {
	file="$1"
	if [ -f "$file" ]; then
		dos2unix $file
		while IFS="=" read -r key value; do
			case "$key" in
			"gdac_scheduler_name") GDAC_SCHEDULER_NAME="$value" ;;
		...
		GDAC_SCHEDULER_NAME=$(echo $SECRET_STRING | jq -r '.SecretString' | jq -r '.gdac_scheduler_name')
		GMC_API_USERNAME=$(echo $SECRET_STRING  | jq -r '.SecretString' | jq -r '.gmc_api_username')
		GMC_API_PASSWORD=$(echo $SECRET_STRING  | jq -r '.SecretString' | jq -r '.gmc_api_password')
		GMC_API_ENDPOINT=$(echo $SECRET_STRING  | jq -r '.SecretString' | jq -r '.gmc_api_endpoint')
		...
		echo "INFO ::: github_organization=$GITHUB_ORGANIZATION :: gdac_scheduler_name=$GDAC_SCHEDULER_NAME :: gmc_api_username=$GMC_API_USERNAME :: gmc_api_password=$GMC_API_PASSWORD :: gmc_api_endpoint=$GMC_API_ENDPOINT :: pem_key_path=$PEM_KEY_PATH"
	fi
...
append_gdac_keys_values_to_tfvars() {
	...
	echo "INFO ::: Append GDAC key-value(s) to tfvars, ::: $outFile"
}
...
validate_secret_values() {
	...
	SECRET_VALUE=""
	if [ "$SECRET_KEY" == "gmc_api_username" ]; then
		SECRET_VALUE=$(aws secretsmanager get-secret-value --secret-id "$SECRET_NAME" --region "${AWS_REGION}" --profile "${AWS_PROFILE}" | jq -r '.SecretString' | jq -r '.gmc_api_username')
	elif [ "$SECRET_KEY" == "gmc_api_password" ]; then
		SECRET_VALUE=$(aws secretsmanager get-secret-value --secret-id "$SECRET_NAME" --region "${AWS_REGION}" --profile "${AWS_PROFILE}" | jq -r '.SecretString' | jq -r '.gmc_api_password')
	elif [ "$SECRET_KEY" == "gdac_product_key" ]; then
		SECRET_VALUE=$(aws secretsmanager get-secret-value --secret-id "$SECRET_NAME" --region "${AWS_REGION}" --profile "${AWS_PROFILE}" | jq -r '.SecretString' | jq -r '.gdac_product_key')
	elif [ "$SECRET_KEY" == "gmc_api_endpoint" ]; then
		SECRET_VALUE=$(aws secretsmanager get-secret-value --secret-id "$SECRET_NAME" --region "${AWS_REGION}" --profile "${AWS_PROFILE}" | jq -r '.SecretString' | jq -r '.gmc_api_endpoint')
...
parse_textfile_for_user_secret_keys_values() {
	...
	while IFS="=" read -r key value; do
		case "$key" in
		"gmc_api_username") GMC_API_USERNAME="$value" ;;
		"gmc_api_password") GMC_API_PASSWORD="$value" ;;
		"gdac_product_key") GDAC_PRODUCT_KEY="$value" ;;
		"gmc_api_endpoint") GMC_API_ENDPOINT="$value" ;;
	...
	validate_kvp gmc_api_username "${GMC_API_USERNAME}"
	validate_kvp gmc_api_password "${GMC_API_PASSWORD}"
	validate_kvp gdac_product_key "${GDAC_PRODUCT_KEY}"
	validate_kvp gmc_api_endpoint "${GMC_API_ENDPOINT}"
...
###########Adding Local IP to Security Group which is realted to GDAC Public IP Address
add_ip_to_sec_grp() {
	GDAC_SCHEDULER_IP_ADDR=$1
	...
	### Get Security group of GDAC Scheduler
	SECURITY_GROUP_ID=$(aws ec2 describe-instances --query "Reservations[].Instances[].{Name:Tags[?Key=='Name']|[0].Value,Status:State.Name,SecurityGroups:SecurityGroups[*]}" --filters "Name=tag:Name,Values='$GDAC_SCHEDULER_NAME'" "Name=instance-state-name,Values=running" --region $AWS_REGION --profile $AWS_PROFILE | grep -e "GroupId" | cut -d":" -f 2 | tr -d '"')
	echo $SECURITY_GROUP_ID
	echo "INFO ::: Security group of $GDAC_SCHEDULER_NAME is $SECURITY_GROUP_ID"
...
ARG_COUNT="$#"
######################## Validating AWS profile for GDAC ####################################
validate_aws_profile() {
	echo "INFO ::: Validating AWS profile ${AWS_PROFILE} for GDAC  . . . . . . . . . . . . . . . . !!!"
	...
	if [[ "$(grep '^[[]profile' <~/.aws/config | awk '{print $2}' | sed 's/]$//' | grep "${AWS_PROFILE}")" == "" ]]; then
		...
		exit 1
	else # AWS Profile global available in JumpBox machine
...
UI_Deployment(){
	...
	ssh -i "$PEM" ubuntu@"$GDAC_SCHEDULER_IP_ADDR" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "sudo apt-get install dos2unix"

	ssh -i "$PEM" ubuntu@"$GDAC_SCHEDULER_IP_ADDR" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "dos2unix ~/UI_deploy_kendra_es/*" 
	RES="$?"
	if [ $RES -ne 0 ]; then
		echo "ERROR ::: Failed to do dos2unix UI_deploy_kendra_es to GDAC_Scheduer Instance."
		exit 1
	elif [ $RES -eq 0 ]; then
		echo "INFO ::: I_deploy_kendra_es folder executed dos2unix Successfully to GDAC_Scheduer Instance."
	fi
	#IAM_USER to be defined. et user
	ssh -i "$PEM" ubuntu@"$GDAC_SCHEDULER_IP_ADDR" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "cd ~/UI_deploy_kendra_es && sudo chmod 755 UI_deployment_kendra.sh && ./UI_deployment_kendra.sh $ANALYTICS_SERVICE $AWS_REGION $AWS_PROFILE $GITHUB_ORGANIZATION $GIT_BRANCH" 

	RES="$?"
	if [ $RES -ne 0 ]; then
		echo "ERROR ::: UI_Deployment :: Failed to execute UI_deployment_kendra.sh to GDAC_Scheduer Instance."
		exit 1
	elif [ $RES -eq 0 ]; then
		echo "INFO ::: UI_Deployment :: UI_deployment_kendra.sh executed Successfully to GDAC_Scheduer Instance."
	fi
...
Schedule_CRON_JOB() {
	...
	echo "INFO ::: Scheduling CRON_JOB :: Public IP Address:- $GDAC_SCHEDULER_IP_ADDR"
	echo "ssh -i "$PEM" ubuntu@$GDAC_SCHEDULER_IP_ADDR -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null"
	### Create TFVARS File for PROVISION_GDAC.SH which is Used by CRON JOB - to Provision GDAC Stack
	CRON_DIR_NAME="${GMC_VOLUME_NAME}_${ANALYTICS_SERVICE}"
	...
	NEW_GDAC_IP=$(echo $GDAC_SCHEDULER_IP_ADDR | tr '.' '-')
	RND=$(( $RANDOM % 1000000 )); 
	LAMBDA_LAYER_SUFFIX=$(echo $RND)
	####AWS command for getting instance-id
	GDACSCHEDULER_UID=$(aws ec2 describe-instances --query "Reservations[].Instances[].InstanceId" --filters "Name=tag:Name,Values='$GDAC_SCHEDULER_NAME'" "Name=instance-state-name,Values=running"  --profile $AWS_PROFILE | jq '.[]'|tr -d '"')
	echo "INFO ::: GDACSCHEDULER_UID :: $GDACSCHEDULER_UID which will be added for lambda layer::: "
	echo "INFO ::: USER_SECRET :: $USER_SECRET ::: "
	echo "aws_profile="\"$AWS_PROFILE\" >>$TFVARS_FILE_NAME
	echo "region="\"$AWS_REGION\" >>$TFVARS_FILE_NAME
	echo "volume_name="\"$GMC_VOLUME_NAME\" >>$TFVARS_FILE_NAME
	...
	echo "gdac_scheduler_name="\"$GDAC_SCHEDULER_NAME\" >>$TFVARS_FILE_NAME
	echo "gdac_es_securitygroup_id="\"$GDAC_ES_SECURITYGROUP_ID\" >>$TFVARS_FILE_NAME
	echo "gdacscheduler_uid="\"$GDACSCHEDULER_UID\" >>$TFVARS_FILE_NAME
	...
	if [ $ARG_COUNT -eq 5 ]; then
		echo "INFO ::: $ARG_COUNT th Argument is supplied as ::: $GDAC_INPUT_KVP"
		append_gdac_keys_values_to_tfvars $GDAC_INPUT_KVP $TFVARS_FILE_NAME
	fi
	###UI deplyment
	UI_Deployment $ANALYTICS_SERVICE $AWS_REGION $AWS_PROFILE $GITHUB_ORGANIZATION $GIT_BRANCH

	scp -i "$PEM" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null create_layer.sh tracker_json.py ubuntu@$GDAC_SCHEDULER_IP_ADDR:~/
	RES="$?"
	if [ $RES -ne 0 ]; then
		echo "ERROR ::: Failed to Copy create_layer.sh to GDAC_Scheduer Instance."
		exit 1
	elif [ $RES -eq 0 ]; then
		echo "INFO ::: create_layer.sh Uploaded Successfully to GDAC_Scheduer Instance."
	fi
	if [ "${ANALYTICS_SERVICE^^}" = "ES" ] || [ "${ANALYTICS_SERVICE^^}" = "OS" ]; then
		# Kendra_UI_Deployment $ANALYTICS_SERVICE $AWS_REGION $AWS_PROFILE
		# scp -i "$PEM" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "$TFVARS_FILE_NAME" ubuntu@$GDAC_SCHEDULER_IP_ADDR:~/
		ssh -i "$PEM" ubuntu@"$GDAC_SCHEDULER_IP_ADDR" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "sudo cp tracker_json.py /var/www/Tracker_UI/docs/"
		RES="$?"
		if [ $RES -ne 0 ]; then
			echo "ERROR ::: Failed to copy tracker_json.py to /var/www/Tracker_UI/docs/."
			exit 1
		elif [ $RES -eq 0 ]; then
			echo "INFO ::: copy tracker_json.py to /var/www/Tracker_UI/docs/ Successfully to GDAC_Scheduer Instance."
		fi	
	fi
	ssh -i "$PEM" ubuntu@"$GDAC_SCHEDULER_IP_ADDR" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "dos2unix create_layer.sh"
	RES="$?"
	if [ $RES -ne 0 ]; then
		echo "ERROR ::: Failed to do dos2unix create_layer.sh to GDAC_Scheduer Instance."
		exit 1
	elif [ $RES -eq 0 ]; then
		echo "INFO ::: create_layer.sh executed dos2unix Successfully to GDAC_Scheduer Instance."
	fi
	#IAM_USER to be defined. et user 
	ssh -i "$PEM" ubuntu@"$GDAC_SCHEDULER_IP_ADDR" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "sh create_layer.sh $AWS_PROFILE $GDACSCHEDULER_UID" 
	RES="$?"
	if [ $RES -ne 0 ]; then
		echo "ERROR ::: Scheduling CRON_JOB :: Failed to execute create_layer.sh to GDAC_Scheduer Instance."
		exit 1
	elif [ $RES -eq 0 ]; then
		echo "INFO ::: create_layer.sh executed Successfully to GDAC_Scheduer Instance."
	fi
	### Create Directory for each Volume
	ssh -i "$PEM" ubuntu@"$GDAC_SCHEDULER_IP_ADDR" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "[ ! -d $CRON_DIR_NAME ] && mkdir $CRON_DIR_NAME "
	KENDRA_TRACKER_JSON_FOLDER="kendra_tracker_json_folder"
	if [[ "${ANALYTICS_SERVICE^^}" = "KENDRA" ]]; then
		#ssh -i "$PEM" ubuntu@"$GDAC_SCHEDULER_IP_ADDR" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "[ ! -d $KENDRA_TRACKER_JSON_FOLDER ] && mkdir $KENDRA_TRACKER_JSON_FOLDER "
		# ES_UI_Deployment
		scp -i "$PEM" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null  tracker_json_kendra.py ubuntu@$GDAC_SCHEDULER_IP_ADDR:~/
		ssh -i "$PEM" ubuntu@"$GDAC_SCHEDULER_IP_ADDR" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "sudo cp tracker_json_kendra.py /var/www/Tracker_UI/docs/"
		RES="$?"
		if [ $RES -ne 0 ]; then
			echo "ERROR ::: Failed to Copy tracker_json_kendra.py to GDAC_Scheduer Instance."
			exit 1
		elif [ $RES -eq 0 ]; then
			echo "INFO ::: tracker_json_kendra.py Uploaded Successfully to GDAC_Scheduer Instance."
		fi
	fi

	### Copy TFVARS and provision_gdac.sh to GDACScheduler
	scp -i "$PEM" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null provision_gdac.sh "$TFVARS_FILE_NAME" ubuntu@$GDAC_SCHEDULER_IP_ADDR:~/$CRON_DIR_NAME

	ssh -i "$PEM" ubuntu@"$GDAC_SCHEDULER_IP_ADDR" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "sudo chmod -R 775 $CRON_DIR_NAME"

	RES="$?"
	if [ $RES -ne 0 ]; then
		echo "ERROR ::: Failed to Copy $TFVARS_FILE_NAME to GDAC_Scheduer Instance."
		exit 1
	elif [ $RES -eq 0 ]; then
		echo "INFO ::: $TFVARS_FILE_NAME Uploaded Successfully to GDAC_Scheduer Instance."
	fi
	rm -rf $TFVARS_FILE_NAME
	#dos2unix command execute
	ssh -i "$PEM" ubuntu@"$GDAC_SCHEDULER_IP_ADDR" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "dos2unix ~/$CRON_DIR_NAME/provision_gdac.sh"
	### Check If CRON JOB is running for a specific VOLUME_NAME
	CRON_VOL=$(ssh -i "$PEM" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null ubuntu@"$GDAC_SCHEDULER_IP_ADDR" "crontab -l |grep \"~/$CRON_DIR_NAME/$TFVARS_FILE_NAME\"")
	if [ "$CRON_VOL" != "" ]; then
		### DO Nothing. CRON JOB takes care of GDAC Provisioning
		echo "INFO ::: crontab does not require volume entry. As it is already present.:::::"
	else
		### Set up a new CRON JOB for GDAC Provisioning

		echo "INFO ::: Setting CRON JOB for $CRON_DIR_NAME as it is not present"
		ssh -i "$PEM" ubuntu@$GDAC_SCHEDULER_IP_ADDR -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "(crontab -l ; echo '*/$FREQUENCY * * * * cd ~/$CRON_DIR_NAME && /bin/bash provision_gdac.sh  ~/$CRON_DIR_NAME/$TFVARS_FILE_NAME') | sort - | uniq - | crontab -"
		if [ $? -eq 0 ]; then
			echo "INFO :::  Scheduling CRON_JOB :: SUCCESS :: for GMC VOLUME and Service :: $CRON_DIR_NAME"
			exit 0
		else
			echo "ERROR :::  Scheduling CRON_JOB :: FAILED :: for GMC VOLUME and Service :: $CRON_DIR_NAME"
			exit 1
		fi
	fi
}
...
if [ $# -eq 0 ]; then
	echo "ERROR ::: No argument(s) supplied. This Script Takes 4 Mandatory Arguments 1) GMC Volume_Name, 2) Service, 3) Frequency and 4) User Secret(either Existing Secret Name Or Secret KVPs in a text file)"
	exit 1
elif [ $# -lt 4 ]; then
	echo "ERROR ::: $# argument(s) supplied. This Script Takes 4 Mandatory Arguments 1) GMC Volume_Name, 2) Service, 3) Frequency and 4) User Secret(either Existing Secret Name Or Secret KVPs in a text file)"
	exit 1
fi
#################### Validate Arguments Passed to GDAC_Scheduler.sh ####################
GMC_VOLUME_NAME="$1"   ### 1st argument  ::: GMC_VOLUME_NAME
...
GDAC_INPUT_KVP="$5"     ### 5th argument  ::: User defined KVP file for passing arguments to GDAC
...
echo "INFO ::: Validating Arguments Passed to GDAC_Scheduler.sh"
if [ "${#GMC_VOLUME_NAME}" -lt 3 ]; then
	echo "ERROR ::: Something went wrong. Please re-check 1st argument and provide a valid GMC Volume Name."
	exit 1
fi
...
if [[ -n "$FOURTH_ARG" ]]; then
	...
	if [ -f "$FOURTH_ARG" ]; then
		...
		### gdac_scheduler_name   - Get the value -- If its not null / "" then GDAC_SCHEDULER_NAME = ${gdac_scheduler_name}
		create_JSON_from_Input_user_KVPfile $FOURTH_ARG >user_creds_"${GMC_VOLUME_NAME}"_"${ANALYTICS_SERVICE}".json
		### Formation of User Secret Name
		USER_SECRET="prod/gdac/admin/$GMC_VOLUME_NAME/$ANALYTICS_SERVICE"
		### Verify the Secret Exists
		...
		if [ "$USER_SECRET_EXISTS" != "N" ]; then
			...
			--secret-string file://user_creds_"${GMC_VOLUME_NAME}"_"${ANALYTICS_SERVICE}".json \
			--region "${AWS_REGION}" --profile "${AWS_PROFILE}"
			RES="$?"
			if [ $RES -ne 0 ]; then
			...
			--description "Preserving User specific data/secrets to be used for GDAC Scheduling" \
			--secret-string file://user_creds_"${GMC_VOLUME_NAME}"_"${ANALYTICS_SERVICE}".json \
			--region "${AWS_REGION}" --profile "${AWS_PROFILE}"
			RES="$?"
			if [ $RES -ne 0 ]; then
		...
		rm -rf *"${GMC_VOLUME_NAME}"_"${ANALYTICS_SERVICE}".json
		...
		if [ "$USER_SECRET_EXISTS" == "Y" ]; then
			...
			validate_secret_values "$USER_SECRET" gmc_api_username "$AWS_REGION" "$AWS_PROFILE"
			validate_secret_values "$USER_SECRET" gmc_api_password "$AWS_REGION" "$AWS_PROFILE"
			validate_secret_values "$USER_SECRET" gdac_product_key "$AWS_REGION" "$AWS_PROFILE"
			validate_secret_values "$USER_SECRET" gmc_api_endpoint "$AWS_REGION" "$AWS_PROFILE"
	...
	echo "INFO ::: Fourth argument is NOT provided, So, It will consider prod/gdac/admin as the default user secret."
fi
######################  GDAC Scheduler Instance is Available ##############################
GDAC_SCHEDULER_NAME=""
### parse_textfile_for_gdac_scheduler_name "$FOURTH_ARG"
parse_4thArgument_for_gdac_scheduler_name "$FOURTH_ARG"

########################Create OS Admin Secret, If its not available ###############

OS_ADMIIN_SECRET="global-labs-os-admin"
### Verify the Secret Exists
...
######################## Check If GDAC_ES_Security Available ###############################################
GDAC_ES_SECURITYGROUP_ID=""
...
Create_GDAC_ES_SecurityGroup $USER_VPC_ID $AWS_PROFILE $AWS_REGION
echo "INFO ::: GDAC_ES_SecurityGroup :: $GDAC_ES_SECURITYGROUP_ID"

########################Create KENDRA Admin Secret, If its not available ###############

KENDRA_ADMIIN_SECRET="global-labs-kendra-admin"
...
if [ "${ANALYTICS_SERVICE^^}" = "ES" ] || [ "${ANALYTICS_SERVICE^^}" = "OS" ]; then
	check_if_opensearch_exists $OS_ADMIIN_SECRET $AWS_REGION $AWS_PROFILE $GITHUB_ORGANIZATION $GDAC_ES_SECURITYGROUP_ID
elif [ "${ANALYTICS_SERVICE^^}" = "KENDRA" ]; then
	check_if_kendra_exists $KENDRA_ADMIIN_SECRET $AWS_REGION $AWS_PROFILE $GITHUB_ORGANIZATION $GDAC_ES_SECURITYGROUP_ID
else
	echo "Invalid Input for KENDRA/ES"
fi

echo "INFO ::: Get IP Address of GDAC Scheduler Instance"
######################  GDAC Scheduler Instance is Available ##############################
...
if [ "$GDAC_SCHEDULER_NAME" != "" ]; then
	### User has provided the GDACScheduler Name as Key-Value from 4th Argument
	if [[ "$USE_PRIVATE_IP" != "Y" ]]; then
		### Getting Public_IP of GDAC Scheduler
		GDAC_SCHEDULER_IP_ADDR=$(aws ec2 describe-instances --query "Reservations[*].Instances[*].{Name:Tags[?Key=='Name']|[0].Value,Status:State.Name,PublicIP:PublicIpAddress}" --filters "Name=tag:Name,Values='$GDAC_SCHEDULER_NAME'" "Name=instance-state-name,Values=running" --region "${AWS_REGION}" --profile ${AWS_PROFILE}| grep -e "PublicIP" | cut -d":" -f 2 | tr -d '"' | tr -d ' ')
		echo "INFO ::: Public_IP of GDAC Scheduler is: $GDAC_SCHEDULER_IP_ADDR"
	else
		### Getting Private_IP of GDAC Scheduler
		echo "INFO ::: Private_IP of GDAC Scheduler is: $GDAC_SCHEDULER_IP_ADDR"
		GDAC_SCHEDULER_IP_ADDR=`aws ec2 describe-instances --query "Reservations[*].Instances[*].{Name:Tags[?Key=='Name']|[0].Value,Status:State.Name,PrivateIp:PrivateIpAddress}" --filters "Name=tag:Name,Values='$GDAC_SCHEDULER_NAME'" "Name=instance-state-name,Values=running" --region "${AWS_REGION}" --profile ${AWS_PROFILE} | grep -e "PrivateIp" | cut -d":" -f 2 | tr -d '"' | tr -d ' '`
	fi
else
	GDAC_SCHEDULER_IP_ADDR=$(aws ec2 describe-instances --query "Reservations[*].Instances[*].{Name:Tags[?Key=='Name']|[0].Value,Status:State.Name,PublicIP:PublicIpAddress}" --filters "Name=tag:Name,Values='GDACScheduler'" "Name=instance-state-name,Values=running" --region "${AWS_REGION}" --profile ${AWS_PROFILE}| grep -e "PublicIP" | cut -d":" -f 2 | tr -d '"' | tr -d ' ')
fi
echo "INFO ::: GDAC_SCHEDULER_IP_ADDR ::: $GDAC_SCHEDULER_IP_ADDR"
if [ "$GDAC_SCHEDULER_IP_ADDR" != "" ]; then
	echo "INFO ::: GDAC Scheduler Instance is Available. IP Address: $GDAC_SCHEDULER_IP_ADDR"
	### Call this function to add Local public IP to Security group of GDAC_SCHEDULER IP
	add_ip_to_sec_grp $GDAC_SCHEDULER_IP_ADDR $GDAC_SCHEDULER_NAME
	###UI development
	# echo "INFO ::: GDAC_SCHEDULER_IP_ADDR :: $GDAC_SCHEDULER_IP_ADDR"
	# PEM="$PEM_KEY_PATH"
	# check_if_pem_file_exists $PEM
	...
	# ssh -i "$PEM" ubuntu@"$GDAC_SCHEDULER_IP_ADDR" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null "[ ! -d $UI_DEPLOY_FOLDER ] && mkdir $UI_DEPLOY_FOLDER "
	# RES="$?"
	# if [ $RES -ne 0 ]; then
	# 	echo "ERROR ::: Failed to create folder $UI_DEPLOY_FOLDER to GDAC_Scheduer Instance."
	# 	exit 1
	# elif [ $RES -eq 0 ]; then
	# 	echo "INFO ::: $UI_DEPLOY_FOLDER folder created Successfully to GDAC_Scheduer Instance."
	# fi
	# scp -i "$PEM" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null UI_deployment_kendra.sh $TFVARS_GDAC_SCHEDULER ubuntu@$GDAC_SCHEDULER_IP_ADDR:~/$UI_DEPLOY_FOLDER
	# RES="$?"
	# if [ $RES -ne 0 ]; then
	# 	echo "ERROR ::: Failed to Copy UI_deployment_kendra.sh $TFVARS_GDAC_SCHEDULER  to GDAC_Scheduer Instance."
	# 	exit 1
	# elif [ $RES -eq 0 ]; then
	# 	echo "INFO ::: UI_deployment_kendra.sh $TFVARS_GDAC_SCHEDULER Uploaded Successfully to GDAC_Scheduer Instance."
	# fi
	# exit 99
	### gmc endpoint accessibility $GDAC_SCHEDULER_NAME $GDAC_SCHEDULER_IP_ADDR
	gmc_endpoint_accessibility  $GDAC_SCHEDULER_NAME $GDAC_SCHEDULER_IP_ADDR $GMC_API_ENDPOINT $GMC_API_USERNAME $GMC_API_PASSWORD #458
	Schedule_CRON_JOB $GDAC_SCHEDULER_IP_ADDR $ANALYTICS_SERVICE

###################### GDAC Scheduler EC2 Instance is NOT Available ##############################
else
	## "GDAC Scheduler is not present. Creating new EC2 machine."
	echo "INFO ::: GDAC Scheduler Instance is not present. Creating new EC2 machine."
	########## Download GDAC Scheduler Instance Provisioning Code from GitHub ##########
	### GITHUB_ORGANIZATION defaults to global-labs
	REPO_FOLDER="global-analyticsconnector-manager"
	validate_github $GITHUB_ORGANIZATION $REPO_FOLDER 
	...
	echo "INFO ::: GDAC Scheduler EC2 provisioning ::: BEGIN - Executing ::: Terraform init . . . . . . . . "
	COMMAND="terraform init"
	$COMMAND
	echo "INFO ::: GDAC Scheduler EC2 provisioning ::: FINISH - Executing ::: Terraform init."
	echo "INFO ::: GDAC Scheduler EC2 provisioning ::: BEGIN - Executing ::: Terraform apply . . . . . . . . . . . . . . . . . . ."
	### Create .tfvars file to be used by the GDACScheduler Instance Provisioning
	current_folder
	TFVARS_GDAC_SCHEDULER="GDACScheduler.tfvars"
	rm -rf "$TFVARS_GDAC_SCHEDULER" 
	...
	cp $PEM_KEY_PATH ./
	chmod 400 $PEM
	echo "aws_profile="\"$AWS_PROFILE\" >>$TFVARS_GDAC_SCHEDULER
	echo "region="\"$AWS_REGION\" >>$TFVARS_GDAC_SCHEDULER
	echo "gdac_es_securitygroup_id="\"$GDAC_ES_SECURITYGROUP_ID\" >>$TFVARS_GDAC_SCHEDULER
	if [[ "$GDAC_SCHEDULER_NAME" != "" ]]; then
		echo "gdac_scheduler_name="\"$GDAC_SCHEDULER_NAME\" >>$TFVARS_GDAC_SCHEDULER
		### Create entries about the Pem Key in the TFVARS File
		echo "pem_key_file="\"$PEM\" >>$TFVARS_GDAC_SCHEDULER
		echo "aws_key="\"$AWS_KEY\" >>$TFVARS_GDAC_SCHEDULER
	fi
	echo "github_organization="\"$GITHUB_ORGANIZATION\" >>$TFVARS_GDAC_SCHEDULER
	echo "git_branch="\"$GIT_BRANCH\" >>$TFVARS_GDAC_SCHEDULER
	if [[ "$VPC_IS" != "" ]]; then
		echo "user_vpc_id="\"$VPC_IS\" >>$TFVARS_GDAC_SCHEDULER
	fi
	if [[ "$SUBNET_IS" != "" ]]; then
		echo "user_subnet_id="\"$SUBNET_IS\" >>$TFVARS_GDAC_SCHEDULER
	fi
	if [[ "$AZ_IS" != "" ]]; then
		echo "subnet_availability_zone="\"$AZ_IS\" >>$TFVARS_GDAC_SCHEDULER
	fi
	if [[ "$USE_PRIVATE_IP" != "" ]]; then
		echo "use_private_ip="\"$USE_PRIVATE_IP\" >>$TFVARS_GDAC_SCHEDULER
	else
		USE_PRIVATE_IP=N
		echo "use_private_ip="\"$USE_PRIVATE_IP\" >>$TFVARS_GDAC_SCHEDULER
	fi
	echo "INFO ::: service_name - $ANALYTICS_SERVICE"
	echo "service_name="\"$ANALYTICS_SERVICE\" >>$TFVARS_GDAC_SCHEDULER
	echo "$TFVARS_GDAC_SCHEDULER created"
	echo `cat $TFVARS_GDAC_SCHEDULER`
	echo "INFO ::: use_private_ip - $USE_PRIVATE_IP"
	echo "INFO ::: user_vpc_id - $VPC_IS"
	echo "INFO ::: user_subnet_id - $SUBNET_IS"

	dos2unix $TFVARS_GDAC_SCHEDULER
	COMMAND="terraform apply -var-file=$TFVARS_GDAC_SCHEDULER -auto-approve"
	$COMMAND
	if [ $? -eq 0 ]; then
		echo "INFO ::: GDAC Scheduler EC2 provisioning ::: FINISH - Executing ::: Terraform apply ::: SUCCESS."
	else
		echo "ERROR ::: GDAC Scheduler EC2 provisioning ::: FINISH - Executing ::: Terraform apply ::: FAILED."
		exit 1
	fi
	ip=$(cat GDACScheduler_IP.txt)
	GDAC_SCHEDULER_IP_ADDR=$ip
	echo 'INFO ::: New pubilc IP just created:-'$ip
	current_folder
	cd ../
	current_folder
	## Call this function to add Local public IP to Security group of GDAC_SCHEDULER IP
	add_ip_to_sec_grp ${GDAC_SCHEDULER_IP_ADDR}
	###UI development
	echo "INFO ::: GDAC_SCHEDULER_IP_ADDR :: $GDAC_SCHEDULER_IP_ADDR"
	PEM="$PEM_KEY_PATH"
	check_if_pem_file_exists $PEM
	...
	scp -i "$PEM" -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null UI_deployment_kendra.sh "${GIT_REPO_NAME}"/$TFVARS_GDAC_SCHEDULER ubuntu@$GDAC_SCHEDULER_IP_ADDR:~/$UI_DEPLOY_FOLDER
	RES="$?"
	if [ $RES -ne 0 ]; then
		echo "ERROR ::: Failed to Copy UI_deployment_kendra.sh $TFVARS_GDAC_SCHEDULER  to GDAC_Scheduer Instance."
		exit 1

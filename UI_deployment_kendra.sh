...
source /home/ubuntu/UI_deploy_kendra_es/${REPO_FOLDER}GDACScheduler.tfvars
echo "INFO ::: gdac-scheduler-name $gdac_scheduler_name"
...
if [[ "$USE_PRIVATE_IP" == "N" ]]; then
	REPO_FOLDER="global-opensearch-userinterface-public"
else
	REPO_FOLDER="global-opensearch-userinterface"
	sed -i '/^$/d' $TFVARS_GDAC_SCHEDULER
	sed -i '/vpc_endpoint_id/d'  $TFVARS_GDAC_SCHEDULER
	...
	echo "vpc_endpoint_id="\"$VPC_ENDPOINT_ID\" >> $TFVARS_GDAC_SCHEDULER
	echo "" >> $TFVARS_GDAC_SCHEDULER
fi
echo "INFO ::: REPO_FOLDER $REPO_FOLDER"
#exit 11
#cp $TFVARS_GDAC_SCHEDULER $REPO_FOLDER/
...
if [[ "$FLAG" == "N" ]]; then
	...
	pwd
	cp ../$TFVARS_GDAC_SCHEDULER ./
	current_folder
        sudo chmod 755 DeployGlobalWeb.sh
        sudo ./DeployGlobalWeb.sh
	#cd "${REPO_FOLDER}"
...
FILE_FLAG="N"
#[ -f "global-opensearch-userinterface-public/search-api*" ] && FILE_FLAG="Y"
SEARCH_API=""
if [ -f "$FILE" ]; then

	echo "$FILE exists."
	U_ID=$(cat $FILE)
	SEARCH_API=$(aws secretsmanager get-secret-value --secret-id global-labs-search-api-$U_ID --region $AWS_REGION --profile $AWS_PROFILE | jq -r '.SecretString' | jq -r '.search_api_endpoint')
...
if [ "${ANALYTICS_SERVICE^^}" = "ES" ] || [ "${ANALYTICS_SERVICE^^}" = "OS" ]; then
	...
	if [ "$SEARCH_API" == "" ] || [ "$SEARCH_API" == "null" ] ; then
		...
		VAR_SEARCHUI_PATH="/var/www/SearchUI_Web"
		      ...
		      echo Global ElasticSearch Web portal: http://$(curl checkip.amazonaws.com)/search/index.html
		      echo Global ElasticSearch Tracker Web portal: http://$(curl checkip.amazonaws.com)/tracker/index.html
		#else
...
#SchName="abc"
	...
	current_folder
sed -i 's#var schedulerName.*$#var schedulerName = \"'${gdac_scheduler_name}'\"; #g' ~/UI_deploy_kendra_es/${REPO_FOLDER}/Tracker_UI/docs/fetch.js
	#sudo rm -rf /var/www/Tracker_UI

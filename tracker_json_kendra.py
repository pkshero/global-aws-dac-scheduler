...
def add_GDAC_activity(most_recent_run, current_state, latest_toc_handle_processed):
    """
    Add GDAC_activity information
    """
    _GDAC_activity = {}
    _GDAC_activity["most_recent_run"] = most_recent_run
    _GDAC_activity["current_state"] = current_state
    _GDAC_activity["latest_toc_handle_processed"] = latest_toc_handle_processed
    return _GDAC_activity
...
def combined_tracker_UI(index_name, index_id, default_url, frequency, user_secret,  created_on, volume, service, most_recent_run, current_state, latest_toc_handle_processed, gdac_scheduler_name,kendra_url):
    """
    tracker_UI: Going to create the dynamic json file
    """ 
    integration_name = volume + "_" + service
    tracker_json_filename = gdac_scheduler_name + "_tracker_KENDRA.json"
    ...
    if is_file_exist(tracker_dir, tracker_json_filename):
        ...
        if integration_name in tracker_json["INTEGRATIONS"].keys():
            # Change only _GDAC_Activity
            tracker_json["INTEGRATIONS"][integration_name]["_GDAC_activity"] = add_GDAC_activity(most_recent_run, current_state, latest_toc_handle_processed)
            ...
            tracker_json["INTEGRATIONS"][integration_name]["_GDAC_activity"] = {}
...
if __name__ == '__main__':
    ...
    gdac_scheduler_name = sys.argv[12]
    kendra_url = sys.argv[13]

    print("Tracker Json ::: Execution Started")
    tracker_json = combined_tracker_UI(index_name, index_id, default_url, frequency, user_secret,  created_on, volume, service, most_recent_run, current_state, latest_toc_handle_processed, gdac_scheduler_name,kendra_url)

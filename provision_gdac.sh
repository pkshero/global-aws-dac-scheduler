...
#############################################################################################
#### This Script Targets GDAC Deployment from any Linux Box
#### Prequisites:
...
LOG_FILE=provision_gdac_$DATE_WITH_TIME.log
(
START=$(date +%s)
{
	TFVARS_FILE=$1
	read_TFVARS() {
		...
		while IFS="=" read -r key value; do
			case "$key" in
				...
				"volume_name") GMC_VOLUME_NAME="$value" ;;
				...
				"gdac_es_securitygroup") GDAC_ES_SECURITYGROUP="$value" ;;
...
generate_tracker_json_kendra(){
	...
	USER_SECRET=$5
	CREATED_ON=$6
	TRACKER_GMC_VOLUME_NAME=$7
	ANALYTICS_SERVICE=$8
	MOST_RECENT_RUN=${9}
	CURRENT_STATE=${10}
	LATEST_TOC_HANDLE_PROCESSED=${11}
	GDAC_SCHEDULER_NAME=$(echo "${12}" | tr -d '"')
	KENDRA_URL=${13}
	#sudo chmod -R 777 /home/ubuntu/kendra_tracker_json_folder/
	sudo chmod -R 777 /var/www/Tracker_UI/docs/

	echo "INFO ::: GDAC_SCHEDULER_NAME $GDAC_SCHEDULER_NAME generate_tracker_json_kendra"
	
	python3 /var/www/Tracker_UI/docs/tracker_json_kendra.py $INDEX_NAME $INDEX_ID $DEFAULT_URL $FREQUENCY $USER_SECRET  $CREATED_ON $TRACKER_GMC_VOLUME_NAME $ANALYTICS_SERVICE $MOST_RECENT_RUN $CURRENT_STATE $LATEST_TOC_HANDLE_PROCESSED $GDAC_SCHEDULER_NAME $KENDRA_URL
...
generate_tracker_json(){
	...
	CREATED_BY=$6
	CREATED_ON=$7
	TRACKER_GMC_VOLUME_NAME=$8
	ANALYTICS_SERVICE=$9
	MOST_RECENT_RUN=${10}
	CURRENT_STATE=${11}
	LATEST_TOC_HANDLE_PROCESSED=${12}
	GDAC_SCHEDULER_NAME=$(echo "${13}" | tr -d '"')
	sudo chmod -R 777 /var/www/Tracker_UI/docs/
	python3 /var/www/Tracker_UI/docs/tracker_json.py $OS_URL $KIBANA_URL $DEFAULT_URL $FREQUENCY $USER_SECRET $CREATED_BY $CREATED_ON $TRACKER_GMC_VOLUME_NAME $ANALYTICS_SERVICE $MOST_RECENT_RUN $CURRENT_STATE $LATEST_TOC_HANDLE_PROCESSED $GDAC_SCHEDULER_NAME
	echo "INFO ::: TRACKER JSON  Updated"
}
...
validate_github() {
	...
	if [[ $GITHUB_ORGANIZATION == "" ]];then
		...
		echo "INFO ::: github_organization not provided as Secret Key-Value pair. So considering global-labs as the default value !!!"
	fi 
...
GMC_VOLUME_NAME=$(echo "$GMC_VOLUME_NAME" | tr -d '"')
GIT_BRANCH=$(echo "$GIT_BRANCH" | tr -d '"')
GITHUB_ORGANIZATION=$(echo "$GITHUB_ORGANIZATION" | tr -d '"')
GDAC_SCHEDULER_NAME=$(echo "$GDAC_SCHEDULER_NAME" | tr -d '"')
GDAC_ES_SECURITYGROUP=$(echo "$GDAC_ES_SECURITYGROUP" | tr -d '"')
SERVICE_NAME=$(echo "$SERVICE_NAME" | tr -d '"')

echo GDAC_SCHDULER_NAME $GDAC_SCHEDULER_NAME
...
if [ "${SERVICE_NAME^^}" = "ES" ] || [ "${SERVICE_NAME^^}" = "OS" ]; then
	...
	OS_ADMIIN_SECRET="global-labs-kendra-admin"
fi
##################################### START TRACKER JSON Creation ###################################################################

echo "GDAC_Activity : Export In Progress"
KENDRA_URL=""
###Req generate_tracker_json.py start put it in if
if [ "${SERVICE_NAME^^}" = "ES" ] || [ "${SERVICE_NAME^^}" = "OS" ]; then
	OS_URL=$(aws secretsmanager get-secret-value --secret-id $OS_ADMIIN_SECRET --region "${AWS_REGION}" --profile "${AWS_PROFILE}" | jq -r '.SecretString' | jq -r '.gdac_es_url')
	KIBANA_URL=$(aws secretsmanager get-secret-value --secret-id $OS_ADMIIN_SECRET --region "${AWS_REGION}" --profile "${AWS_PROFILE}" | jq -r '.SecretString' | jq -r '.gdac_kibana_url')
	DEFAULT_URL="/search/index.html"
	USER_SECRET=$OS_ADMIIN_SECRET
	CREATED_BY=$(aws secretsmanager get-secret-value --secret-id $OS_ADMIIN_SECRET --region "${AWS_REGION}" --profile "${AWS_PROFILE}" | jq -r '.SecretString' | jq -r '.gdac_es_admin_user')
	CREATED_ON=$(date "+%Y%m%d-%H%M%S")
	TRACKER_GMC_VOLUME_NAME=$GMC_VOLUME_NAME
	...
	echo "INFO ::: Nach sheduler name: " ${GDAC_SCHEDULER_NAME}
	JSON_FILE_PATH="/var/www/Tracker_UI/docs/${GDAC_SCHEDULER_NAME}_tracker_ES.json"
	echo "INFO ::: JSON_FILE_PATH:" $JSON_FILE_PATH
	if [ -f "$JSON_FILE_PATH" ] ; then
		TRACEPATH="${GMC_VOLUME_NAME}_${ANALYTICS_SERVICE}"
		TRACKER_JSON=$(cat $JSON_FILE_PATH)
		echo "Tracker json" $TRACKER_JSON
		LATEST_TOC_HANDLE_PROCESSED=$(echo $TRACKER_JSON | jq -r .INTEGRATIONS.\"$TRACEPATH\"._GDAC_activity.latest_toc_handle_processed)
	...
	generate_tracker_json $OS_URL $KIBANA_URL $DEFAULT_URL $FREQUENCY $USER_SECRET $CREATED_BY $CREATED_ON $TRACKER_GMC_VOLUME_NAME $ANALYTICS_SERVICE $MOST_RECENT_RUN $CURRENT_STATE $LATEST_TOC_HANDLE_PROCESSED $GDAC_SCHEDULER_NAME
	...
	LATEST_TOC_HANDLE_PROCESSED="-"
	echo "INFO ::: Nach sheduler name: " ${GDAC_SCHEDULER_NAME}
	#JSON_FILE_PATH="/home/ubuntu/kendra_tracker_json_folder/${GDAC_SCHEDULER_NAME}_tracker_KENDRA.json"
	JSON_FILE_PATH="/var/www/Tracker_UI/docs/${GDAC_SCHEDULER_NAME}_tracker_KENDRA.json"
	...
	generate_tracker_json_kendra $INDEX_NAME $INDEX_ID $DEFAULT_URL $FREQUENCY $USER_SECRET  $CREATED_ON $TRACKER_GMC_VOLUME_NAME $ANALYTICS_SERVICE $MOST_RECENT_RUN $CURRENT_STATE $LATEST_TOC_HANDLE_PROCESSED $GDAC_SCHEDULER_NAME $KENDRA_URL
...
########## Download GDAC Provisioning Code from GitHub ##########
### GITHUB_ORGANIZATION defaults to global-labs
# REPO_FOLDER="global-analyticsconnector-opensearch"
if [ "${SERVICE_NAME^^}" = "ES" ] || [ "${SERVICE_NAME^^}" = "OS" ]; then 
	if [ "$USE_PRIVATE_IP" == "N" ] || [ "$USE_PRIVATE_IP" == null ] || [ "$USE_PRIVATE_IP" == "" ]; then
        	REPO_FOLDER="global-analyticsconnector-opensearch-public"
	else
	        REPO_FOLDER="global-analyticsconnector-opensearch"
	fi
else
	REPO_FOLDER="global-analyticsconnector-kendra"
fi
echo "INFO ::: REPO_FOLDER - $REPO_FOLDER !!!"
validate_github $GITHUB_ORGANIZATION $REPO_FOLDER
########################### Git Clone : GDAC Provisioning Repo ###############################################################
...
if [ $RESULT -eq 0 ]; then
	...
	echo "ERROR ::: Unable to Proceed with GDAC Provisioning."
	exit 1
fi
pwd
########################### Completed - Git Clone  ###############################################################
echo "INFO ::: Copy TFVARS file to /$(pwd)/${GIT_REPO_NAME}/${TFVARS_FILE}"
# cp "$GMC_VOLUME_NAME/${TFVARS_FILE}" $(pwd)/"${GIT_REPO_NAME}"/
cp "${TFVARS_FILE}" "${GIT_REPO_NAME}"/
cd "${GIT_REPO_NAME}"
pwd

GMC_VOLUME_NAME_1=$(echo $GMC_VOLUME_NAME|tr -d '"')
ANALYTICS_SERVICE_1=$(echo $ANALYTICS_SERVICE|tr -d '"')
GDAC_SCHEDULER_NAME_1=$(echo $GDAC_SCHEDULER_NAME|tr -d '"')

#JSON_FILE_PATH="$HOME/TrackerJson/${GDAC_SCHEDULER_NAME_1}_tracker.json"
...
if [ "${SERVICE_NAME^^}" = "ES" ] || [ "${SERVICE_NAME^^}" = "OS" ]; then
	...
	LATEST_TOC_HANDLE=""
	if [ -f "$JSON_FILE_PATH" ] ; then
		TRACEPATH="${GMC_VOLUME_NAME_1}_${ANALYTICS_SERVICE_1}"
		echo $TRACEPATH
		TRACKER_JSON=$(cat $JSON_FILE_PATH)
		echo "Tracker json" $TRACKER_JSON
		LATEST_TOC_HANDLE=$(echo $TRACKER_JSON | jq -r .INTEGRATIONS.\"$TRACEPATH\"._GDAC_activity.latest_toc_handle_processed)
...
##exit 99
##### RUN terraform init
echo "INFO ::: GDAC provisioning ::: BEGIN - Executing ::: Terraform init."
COMMAND="terraform init"
$COMMAND
chmod 755 $(pwd)/*
# exit 1
echo "INFO ::: GDAC provisioning ::: FINISH - Executing ::: Terraform init."
echo "INFO ::: GDAC provisioning ::: BEGIN - Executing ::: Terraform Apply . . . . . . . . . . . "
COMMAND="terraform apply -var-file=${TFVARS_FILE} -auto-approve"
# COMMAND="terraform validate"
$COMMAND
if [ $? -eq 0 ]; then
	echo "INFO ::: GDAC provisioning ::: FINISH ::: Terraform apply ::: SUCCESS"
	
	######Req for generate_tracker_json for kendra if condition
	if [ "${SERVICE_NAME^^}" = "ES" ] || [ "${SERVICE_NAME^^}" = "OS" ]; then

		echo "GDAC_Activity : Export Completed. Indexing in Progress"
		...
		echo "INFO ::: LATEST_TOC_HANDLE_PROCESSED for GDAC Discovery is : $LATEST_TOC_HANDLE_PROCESSED"
		generate_tracker_json $OS_URL $KIBANA_URL $DEFAULT_URL $FREQUENCY $USER_SECRET $CREATED_BY $CREATED_ON $TRACKER_GMC_VOLUME_NAME $ANALYTICS_SERVICE $MOST_RECENT_RUN $CURRENT_STATE $LATEST_TOC_HANDLE_PROCESSED $GDAC_SCHEDULER_NAME
	else
	...
	echo "INFO ::: GDAC provisioning ::: FINISH ::: Terraform apply ::: FAILED"

	######Req for generate_tracker_json for kendra if condition
	if [ "${SERVICE_NAME^^}" = "ES" ] || [ "${SERVICE_NAME^^}" = "OS" ]; then

		echo "GDAC_Activity : Export Failed/Indexing Failed"
    ...
    echo "GDAC_Activity : Indexing Completed"
    MOST_RECENT_RUN=$(date "+%Y:%m:%d-%H:%M:%S")
    CURRENT_STATE="Indexing-Completed"

    INTERNAL_SECRET=$(head -n 1 gdac_uniqui_id.txt  | tr -d "'")
    echo "INFO ::: Internal secret for GDAC Discovery is : $INTERNAL_SECRET"
...
##Get the GDAC discovery lambda function name
	...
	if [ -z "$DISCOVERY_LAMBDA_NAME"  ]; then
		...
		while [ "$LAST_UPDATE_STATUS" != "InProgress" ]; do
			...
			if [ "$LAST_UPDATE_STATUS" == "Successful" ]; then
				echo "INFO ::: Lambda execution COMPLETED. Preparing for cleanup of GDAC Stack and dependent resources . . . . . . . . . . "
				CLEANUP="Y"
				break
			elif [ "$LAST_UPDATE_STATUS" == "Failed" ]; then
				echo "INFO ::: Lambda execution FAILED. Preparing for cleanup of GDAC Stack and dependent resources . . . . . . . . . .  "
				CLEANUP="Y"
				break
...
echo "INFO ::: STARTED ::: CLEANUP GDAC STACK and dependent resources . . . . . . . . . . . . . . . . . . . . ."

pwd
echo "INFO ::: pwd" pwd
UNIQUE_ID=$(cat gdac_uniqui_id.txt | cut -d - -f4)
echo "INFO ::: UNIQUE_ID ::: $UNIQUE_ID "

aws s3 rm --recursive s3://global-share-data-bucket-storage/gmc_api_data_$UNIQUE_ID/ --profile ${AWS_PROFILE}
if [ $? -eq 0 ]; then
        echo "INFO ::: deleted files from global-share-data-bucket-storage bucket and folder gmc_api_data_$UNIQUE_ID"      
else
        echo "INFO ::: Error in deleted files from global-share-data-bucket-storage bucket and folder gmc_api_data_$UNIQUE_ID"
fi

# ##### RUN terraform destroy to CLEANUP GDAC STACK and dependent resources

COMMAND="terraform destroy -var-file=${TFVARS_FILE} -auto-approve"
$COMMAND
echo "INFO ::: COMPLETED ::: CLEANUP GDAC STACK and dependent resources ! ! ! ! "
#    exit 0
#fi
END=$(date +%s)
secs=$((END - START))
...
#exit 0
	...
	echo "INFO ::: Total execution Time ::: $DIFF"
	exit 0
	echo "INFO ::: Failed GDAC Povisioning"

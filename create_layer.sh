...
####./creater_layer.sh my-layer global
### Comment for test
path="app"
LAYER_NAME="global-labs-os-lambda-layer"
AWS_PROFILE="$1"
GDACSCHEDULER_UID="$2"
...
echo "INFO ::: GDACSCHEDULER_UID ::: $GDACSCHEDULER_UID"
# NEW_GDACSCHEDULER_UID=$(echo $GDAC_IP | tr '.' '-')
# LAMBDA_LAYER_NAME=$(echo $LAYER_NAME-${GDACSCHEDULER_UID^^})
LAMBDA_LAYER_NAME=$(echo $LAYER_NAME-${GDACSCHEDULER_UID})
...
if [ "$EXT_LAMBDA_LAYER" != "$LAMBDA_LAYER_NAME" ]; then
    ...
    aws lambda publish-layer-version --layer-name "${LAMBDA_LAYER_NAME}" --description "Lambda layer for including all pkgs for GDAC_Discovery" --license-info "MIT" --content S3Bucket="${LAMBDA_LAYER_NAME}",S3Key=lambdalayer.zip --compatible-runtimes python3.8 python3.9 --profile $AWS_PROFILE
else
